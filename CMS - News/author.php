            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  <?php    
                    ...
                    $sql = "SELECT `post`.`post_id`, `post`.`title`,`post`.`author`, `post`.`description`, `post`.`post_img`, `post`.`post_date`, `post`.`category`, `category`.`category_name`, `user`.`username` FROM `post` LEFT JOIN `category` ON `post`.`category` = `category`.`category_id` LEFT JOIN `user` ON `post`.`author` = `user`.`user_id` WHERE `post`.`author` = {$auth_id} ORDER BY `post`.`post_id` DESC LIMIT {$offset},{$limit}";
